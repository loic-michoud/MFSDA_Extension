...
def run_stats(y_design, coord_mat, design_data, var_type):
    ...
    for pp in range(p-1):
        ...
        print('coord_mat', coord_mat, 'x_design', x_design, 'y_design', y_design, 'cdesign', cdesign, 'gstat', gstat, 'num_bstrp', num_bstrp, 'thres', thres, 'area', area)
        gpval, clu_pval = bstrp_pvalue(coord_mat, x_design, y_design, cdesign, gstat, num_bstrp, thres, area)
        print('gpval', gpval)
        print('clu_pval', clu_pval)
        gpvals[0, pp] = gpval
        areas[0, pp] = area
